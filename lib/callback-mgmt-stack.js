...
class CallbackMgmtStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const dataStream = scope.node.tryGetContext('data-stream');
      ...
      reason: 'Lambda execution roles need ability to write any LogStream events to its CloudWatch LogGroup.'
		...
		const callbacksTbl = new Table(this, 'SIMPLE', {
			tableName: `${this.stackName}-callback-requests`,
    ...
    const processKinesisEventFn = 'process-kinesis-event';
    this.getFunction(
      processKinesisEventFn,
      processKinesisEventFn,
      this.getRole(processKinesisEventFn, [ kinesisPolicy, dbUpdatePolicy ]),
      Runtime.PYTHON_3_10,
      {
        CALL_TYPE_CB: 'CallBack',
        CB_STATUS: 'PENDING',
        LOGGING_LEVEL: 'INFO',
        TABLE_NAME: callbacksTbl.tableName
      }
    );
