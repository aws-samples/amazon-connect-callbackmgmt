...
const { Runtime, StartingPosition, Tracing } = require('aws-cdk-lib/aws-lambda');
const { NodejsFunction } = require('aws-cdk-lib/aws-lambda-nodejs');
const { join, normalize } = require('path');
const { CfnIntegrationAssociation } = require('aws-cdk-lib/aws-connect');
...
class CallbackStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    const connectInstanceArn = 'arn:aws:connect:' + this.region + ':' + this.account + ':instance/' + instanceId;
    ...
    connectInstanceArn,
		...
		const myFunction = new NodejsFunction(this, `${id}-fn`, {
    ...
    {
      new CfnIntegrationAssociation(this, `${id}-connect-intg`, {
        instanceId: connectInstanceArn,
        integrationArn: myFunction.functionArn,
