...
const handler = async (event) => {
    ...
    console.log('write-callback-fn: event =', event);
    ...
    console.log(`write-callback-fn: phoneNumber = [${phoneNumber}], queue = [${queue}]`);
    ...
    console.log('write-callback-fn: time = [', time, '], date = [', date, ']');

    //DDB TTL 
    const ts = Date.now();
    const ttl= Math.floor(ts/1000) + (+process.env.TTL_DAYS * 86400)   
    console.log(`write-callback-fn: ts = [${ts}], ttl = [${ttl}]`);
    ...
    console.log('write-callback-fn: req =', JSON.stringify(req));
    const resp = await db.send(new PutItemCommand(req));
    console.log('write-callback-fn: resp =', resp);
