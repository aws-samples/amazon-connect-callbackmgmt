...
const handler = async (event) => {
  ...
  console.log('update-callback-fn: event =', event);
  ...
  console.log(`update-callback-fn: phoneNo = [${phoneNo}], queue = [${queue}], cbStatus = [${cbStatus}]`);
  ...
  console.log('update-callback-fn: req =', JSON.stringify(req));
  const resp = await db.send(new UpdateItemCommand(req));
  console.log('update-callback-fn: resp =', resp);
